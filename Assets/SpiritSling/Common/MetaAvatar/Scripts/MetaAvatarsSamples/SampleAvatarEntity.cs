...
public class SampleAvatarEntity : OvrAvatarEntity
{
    ...
    private CAPI.ovrAvatar2Graph _graph = CAPI.ovrAvatar2Graph.Invalid;
    ...
    {
        _graph = CAPI.ovrAvatar2Graph.Oculus;
    ...
    public void LoadRemoteUserCdnAvatar(ulong userId, CAPI.ovrAvatar2Graph graph = CAPI.ovrAvatar2Graph.Oculus)
    {
    ...
    public void SetAvatarAsset(string id)
    {
        _assets.Clear();
        _assets.Add(new AssetData { source = AssetSource.Zip, path = id });
    }
    ...
    protected virtual IEnumerator Retry_HasAvatarRequest()
    {
        ...
        do
        {
            var hasAvatarRequest = OvrAvatarManager.Instance.UserHasAvatarAsync(_userId, _graph);
